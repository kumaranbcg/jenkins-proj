...
/**
CREATE OR REPLACE VIEW `VCA02_LATEST_REPORT_VIEW` AS (
    SELECT 
        `vcaReportView`.* 
    ...
    INNER JOIN (
        ...
        GROUP BY `VCA01_ACTIVITY_MASTER_D`, `VCA02_COMMODITY_MASTER_D`
    ) as `latestVersion`
    ON `vcaReportView`.`activityId` = `latestVersion`.`activityId` AND `vcaReportView`.`commodityId` = `latestVersion`.`commodityId` AND `vcaReportView`.`version` = `latestVersion`.`version`
)
 */
